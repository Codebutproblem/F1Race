// src/pages/GiaiThuongTayDuaPage.jsx
...
const GiaiThuongTayDuaPage = () => {
  const { id } = useParams();
  const [tayDua, setTayDua] = useState(null);
  ...
  const fetchData = async () => {
    try {
      ...
      const allTayDuas = await tayDuaService.getAllTayDua();
      const tayDuaData = allTayDuas.find(td => td.id == id);
      
      if (!tayDuaData) {
        setError('Không tìm thấy thông tin tay đua');
        setLoading(false);
        return;
      }
      
      setTayDua(tayDuaData);
  ...
  const handleOpenThanhToanForm = (giaiThuong) => {
    ...
    setThanhToanFormData({
      ...
      noiDung: `Thanh toán giải thưởng "${giaiThuong.giaiThuong?.ten || ''}" cho tay đua "${tayDua.thanhVien?.ten || ''}"`,
      ngayThanhToan: new Date().toISOString().split('T')[0],
  ...
  const mockPaymentHistory = (giaiThuongId) => [
    { id: 1, ngayThanhToan: '2023-05-15', soTien: 15000000, noiDung: 'Thanh toán đợt 1', phuongThucThanhToan: { ten: 'Chuyển khoản' } },
    { id: 2, ngayThanhToan: '2023-06-20', soTien: 15000000, noiDung: 'Thanh toán đợt 2', phuongThucThanhToan: { ten: 'Chuyển khoản' } }
  ];
        ...
        <main className="flex-1 overflow-y-auto bg-gray-50 p-6">
          ...
          <div className="flex justify-between items-center mb-6">
            <h1 className="text-2xl font-bold text-gray-800">
              Giải thưởng của tay đua: {tayDua?.thanhVien?.ten || ''}
            </h1>
          </div>
          ...
          {loading ? (
            ...
            <>
              ...
              <div className="bg-white rounded-lg shadow-md p-6 mb-6">
                <h2 className="text-xl font-semibold text-gray-700 mb-4">Thông tin tay đua</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <p className="text-gray-600 mb-1">ID:</p>
                    <p className="font-medium">{tayDua.id}</p>
                  </div>
                  <div>
                    <p className="text-gray-600 mb-1">Số năm kinh nghiệm:</p>
                    <p className="font-medium">{tayDua.namKinhNghiem} năm</p>
                  </div>
                  ...
                  {tayDua.thanhVien && (
                    <>
                      <div>
                        ...
                        <p className="font-medium">{tayDua.thanhVien.ten}</p>
                      </div>
                      <div>
                        <p className="text-gray-600 mb-1">Email:</p>
                        <p className="font-medium">{tayDua.thanhVien.email}</p>
                      </div>
                      <div>
                        <p className="text-gray-600 mb-1">Điện thoại:</p>
                        <p className="font-medium">{tayDua.thanhVien.dienThoai}</p>
                      </div>
                      <div>
                        <p className="text-gray-600 mb-1">Ngày sinh:</p>
                        <p className="font-medium">{new Date(tayDua.thanhVien.ngaySinh).toLocaleDateString()}</p>
                      </div>
                    </>
                  )}
                </div>
              ...
              {tayDua.giaiThuongTayDuas && tayDua.giaiThuongTayDuas.length > 0 ? (
                <div className="space-y-6">
                  {tayDua.giaiThuongTayDuas.map((giaiThuong) => (
                ...
                <div className="bg-white rounded-lg shadow-md p-6 text-center">
                  <p className="text-gray-500">Tay đua này chưa có giải thưởng nào</p>
                </div>
              )}
            </>
          ) : (
            <div className="bg-yellow-100 border border-yellow-400 text-yellow-700 px-4 py-3 rounded">
              Không tìm thấy thông tin tay đua với ID: {id}
            </div>
          )}
        </main>
      </div>
...
export default GiaiThuongTayDuaPage;